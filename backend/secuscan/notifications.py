...
async def get_active_rules() -> List[Dict[str, Any]]:
    ...
    from .database import get_db  # <-- Moved inside here

    db = await get_db()
    rows = await db.fetchall("SELECT * FROM notification_rules WHERE is_active = 1")
...
def redact_finding_payload(finding: Dict[str, Any]) -> Dict[str, Any]:
    """
    Redact sensitive data from a finding payload completely.
    """
    from .config import settings  # <-- Moved inside here
    ...
    redacted = {
        ...
        # Overwrite the text fields entirely to shield raw client keys or paths
        "description": "[REDACTED FOR SECURITY]",
        "remediation": "[REDACTED FOR SECURITY]",
        "cvss": finding.get("cvss"),
        "cve": finding.get("cve"),
        "dashboard_url": (
            f"{settings.base_url}/findings/{finding.get('id')}"
...
async def send_email_notification(
    email: str, payload: Dict[str, Any]
) -> tuple[bool, Optional[str]]:
    """
...
async def dispatch_notification(
    rule: Dict[str, Any], finding: Dict[str, Any]
) -> tuple[bool, Optional[str]]:
    """
...
async def record_notification_history(
    rule_id: str, finding_id: str, status: str, error_message: Optional[str] = None
):
...
async def process_notifications_for_finding(finding: Dict[str, Any]):
    ...
    if finding_severity not in ["high", "critical"]:
        logger.debug(
            f"Finding severity {finding_severity} below threshold, skipping notification"
        )
        return
    ...
    logger.info(
        f"Processing notifications for finding {finding_id} with {len(rules)} active rules"
    )
    ...
    for rule in rules:
        ...
        if not severity_matches(finding_severity, rule_threshold):
            logger.debug(
                f"Finding severity {finding_severity} below rule threshold {rule_threshold}"
            )
            continue
        ...
        if await is_duplicate_notification(rule_id, finding_id):
            logger.debug(
                f"Duplicate notification for rule {rule_id} and finding {finding_id}, skipping"
            )
            continue
        ...
        if success:
            logger.info(
                f"Successfully sent notification for rule {rule_id} and finding {finding_id}"
            )
        else:
            logger.error(
                f"Failed to send notification for rule {rule_id} and finding {finding_id}: {error_msg}"
            )
...
async def process_notifications(findings: List[Dict[str, Any]]):
    ...
    tasks = [process_notifications_for_finding(finding) for finding in findings]
