...
@router.get(
    ...
    dependencies=[Depends(read_heavy_limiter)],
)
...
@router.post(
    "/task/start",
    dependencies=[Depends(task_start_limiter), Depends(check_scan_rate_limit)],
)
...
async def retry_task(
    ...
    """
    db = await get_db()
    task = await require_owned_task(
        db, task_id, owner, columns="id, owner_id, status, plugin_id"
    )
    ...
    if not plugin:
        raise HTTPException(
            status_code=404, detail=f"Plugin not found: {task['plugin_id']}"
        )
    ...
    if not can_acquire:
        await executor.mark_task_failed(
            task_id, reason="Concurrency limit reached; task was not retried"
        )
        raise HTTPException(status_code=503, detail=error_msg)
    ...
    return {"task_id": task_id, "status": "queued", "message": "Task retry initiated"}
        ...
        try:
            ...
            if current_status and current_status["status"] in [
                ...
                try:
                    ...
                    if task_row and task_row["raw_output_path"]:
                        for chunk in iter_raw_output_chunks(
                            task_row["raw_output_path"]
                        ):
                            yield {
                                "event": "output",
...
async def get_task_result(task_id: str, owner: str = Depends(get_current_owner)):
    ...
    result = {
        ...
        "errors": [{"message": redact(task_row["error_message"])}]
        if task_row["error_message"]
        else [],
        "error_message": redact(task_row["error_message"])
        if task_row["error_message"]
        else None,
...
async def get_dashboard_summary(owner: str = Depends(get_current_owner)):
    ...
    async def build():
        ...
        async def query_or_default(
            label: str, query_fn: Callable[[], Any], default: Any
        ) -> Any:
            try:
                return await query_fn()
            except Exception as exc:
                logger.warning(
                    "Dashboard summary query '%s' failed for owner %s: %s",
                    label,
                    owner,
                    exc,
                )
        ...
        total_findings = (
            total_findings_row["total"]
            if total_findings_row
            else sum(severity_counts.values())
        )
...
async def get_findings(
    ...
    return await get_or_set_cached(
        f"findings:list:{owner}:page={page}:per_page={per_page}", build
    )
...
async def get_finding_groups(
    ...
    return await get_or_set_cached(
        f"findings:groups:{owner}:page={page}:per_page={per_page}", build
    )
    ...
    async with db.transaction():
        ...
        for i in range(0, len(task_ids), SQLITE_CHUNK_SIZE):
            ...
            await db.execute_no_commit(
                f"DELETE FROM notification_history WHERE finding_id IN (SELECT id FROM findings WHERE task_id IN ({placeholders}))",
                tuple(chunk),
            )
            await db.execute_no_commit(
            ...
            await db.execute_no_commit(
                f"DELETE FROM crawl_runs           WHERE task_id IN ({placeholders})",
                tuple(chunk),
            )
            await db.execute_no_commit(
                f"DELETE FROM asset_services       WHERE task_id IN ({placeholders})",
                tuple(chunk),
            )
            await db.execute_no_commit(
...
@router.patch(
    "/credential-profiles/{profile_id}", dependencies=[Depends(admin_limiter)]
)
...
async def create_workflow(
    ...
    if schedule_seconds is not None:
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid schedule_seconds, must be an integer >= 60",
            )
    else:
...
@router.post(
    "/workflows/{workflow_id}/run", dependencies=[Depends(check_scan_rate_limit)]
)
async def run_workflow_once(workflow_id: str, owner: str = Depends(get_current_owner)):
    ...
    for step in steps:
        ...
        if not can_acquire:
            ...
            logger.warning(
                "Workflow %s: concurrency limit reached for step %s",
                workflow_id,
                step.get("plugin_id"),
            )
            continue
...
async def list_workflow_versions(
    workflow_id: str, owner: str = Depends(get_current_owner)
):
...
async def rollback_workflow(
    workflow_id: str, version_number: int, owner: str = Depends(get_current_owner)
):
...
async def update_workflow(
    workflow_id: str, payload: Dict[str, Any], owner: str = Depends(get_current_owner)
):
    db = await get_db()
    row = await _verify_workflow_owner(db, workflow_id, owner)
    ...
    if enabled_changed:
        await db.log_audit(
            event_type=("workflow_enabled" if new_enabled else "workflow_disabled"),
            message=(
                f"Workflow {workflow_id} {'enabled' if new_enabled else 'disabled'}"
            ),
            context={
                "workflow_id": workflow_id,
                "actor": owner,
...
@router.post(
    "/workflows/scheduler/tick",
    dependencies=[Depends(scheduler_tick_limiter), Depends(check_scan_rate_limit)],
)
...
async def create_notification_rule(
    payload: NotificationRuleCreate, owner: str = Depends(get_current_owner)
):
...
async def _verify_notification_rule_owner(db, rule_id: str, owner: str):
    ...
    if row["owner_id"] != owner:
        raise HTTPException(
            status_code=403, detail="You do not have access to this notification rule"
        )
    return row
...
async def delete_notification_rule(
    rule_id: str, owner: str = Depends(get_current_owner)
):
    db = await get_db()
...
async def upsert_scan_webhook_settings(
    ...
    target = _validate_notification_target(
        NotificationChannelType.WEBHOOK, payload.webhook_url
    )
    db = await get_db()
