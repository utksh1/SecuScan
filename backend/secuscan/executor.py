...
class TaskExecutor:
    ...
    async def _persist_finding(
        ...
        *,
        ...
        # Redact secrets from free-text fields before any DB write.
        # description, remediation, and proof are the only fields that can
        # carry arbitrary scanner output containing credentials or tokens.
        safe_description = redact(str(finding.get("description", "") or ""))
        safe_remediation = redact(str(finding.get("remediation", "") or ""))
        safe_proof = redact(finding.get("proof")) if isinstance(finding.get("proof"), str) else finding.get("proof")
    ...
    async def _upsert_findings_and_report(self, db, task_id: str, owner_id: str, plugin, plugin_id: str, target: str, status: str, output: str = ""):
        ...
        # Redact the entire structured result before writing to tasks.structured_json
        # so that no credential or secret leaks through the JSON persistence path.
        safe_structured_json = json.dumps(redact_dict(structured_result))

        await db.execute(
            "UPDATE tasks SET structured_json = ? WHERE id = ?",
            (safe_structured_json, task_id)
        )
