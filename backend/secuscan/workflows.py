...
class WorkflowScheduler:
    ...
    async def tick(self):
        ...
        for row in rows:
            ...
            (
                wf_rate_ok,
                wf_rate_msg,
            ) = await workflow_rate_limiter.check_workflow_rate_limit(
                row["id"], settings.workflow_min_interval_seconds
            )
            if not wf_rate_ok:
                logger.warning(
                    "Workflow %s skipped by rate limiter: %s", row["id"], wf_rate_msg
                )
                continue

            owner_id = row["owner_id"]
            await self._run_workflow(
                row["id"], json.loads(row.get("steps_json") or "[]"), owner_id=owner_id
            )
            await db.execute(
    ...
    async def _run_workflow(
        self, workflow_id: str, steps: List[Dict[str, Any]], owner_id: str = "default"
    ):
        ...
        for step in steps:
            ...
            target_policy = await get_target_policy(
                db, owner_id, execution_context.get("target_policy_id")
            )
            safe_mode = bool(
            ...
            if not plugin:
                logger.warning(
                    "Workflow %s: plugin %s not found, skipping step",
                    workflow_id,
                    plugin_id,
                )
                continue
            ...
            if target := effective_inputs.get("target"):
                ...
                if plugin.category != "code":
                    try:
                        ...
                        if not is_valid:
                            logger.warning(
                                "Workflow %s: target validation failed for step %s: %s",
                                workflow_id,
                                plugin_id,
                                error_msg,
                            )
                        ...
                        logger.warning(
                            "Workflow %s: target validation timed out for step %s",
                            workflow_id,
                            plugin_id,
                        )
                        continue
                    ...
                    if settings.enforce_network_policy and target_str:
                        ...
                        if not allowed:
                            logger.warning(
                                "Workflow %s: network policy denied %s: %s",
                                workflow_id,
                                target_str,
                                reason,
                            )
            ...
            max_per_hour = (
                ...
                if plugin
                else settings.max_tasks_per_hour
            )
            can_exec, rate_err = await rate_limiter.can_execute(
                plugin_id, max_per_hour, client_id=client
            )
            if not can_exec:
                logger.warning(
                    "Workflow %s: rate limit exceeded for %s: %s",
                    workflow_id,
                    plugin_id,
                    rate_err,
                )
            ...
            if not can_acquire:
                ...
                logger.warning(
                    "Workflow %s: concurrency limit reached for %s",
                    workflow_id,
                    plugin_id,
                )
                continue
