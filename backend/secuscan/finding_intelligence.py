...
def _guess_asset_ref(finding: Dict[str, Any], target: str) -> str:
    asset_refs = (
        finding.get("asset_refs") if isinstance(finding.get("asset_refs"), list) else []
    )
    if asset_refs:
        first = asset_refs[0]
...
def _build_confidence_reason(
    *,
    ...
    if corroborating_sources:
        parts.append(
            f"corroborated by {len(corroborating_sources)} source{'s' if len(corroborating_sources) != 1 else ''}"
        )
...
def _finding_kind_for(finding: Dict[str, Any]) -> str:
    ...
    if (
        ...
        or finding.get("cve")
        or finding.get("validation_method") == "cpe_cve_correlation"
    ):
...
def _source_quality(sources: Iterable[str]) -> float:
    values = [
        _SOURCE_QUALITY.get(str(source).lower(), 0.58)
...
def _compute_confidence(
    ...
    *,
    ...
    validation_component = (
        0.12 if finding.get("validated") else 0.04 if finding.get("cve") else 0.0
    )
    severity_component = {
        "critical": 0.08,
        "high": 0.06,
...
async def normalize_and_correlate_findings(
    ...
    *,
    ...
    for raw_finding in findings:
        ...
        finding_group_id = _stable_id(
            "group", plugin_id, asset_id, _issue_signature(finding)
        )
        base_source = (
            str(
        ...
        normalized_evidence = _dedupe_evidence(
            [
                _typed_evidence(
                    item, source=base_source, observed_at=observed_at, confidence=0.72
                )
                for item in (
                ...
                "asset_refs": sorted(
                    {
                        ...
                        *[
                            str(ref)
                            for ref in finding.get("asset_refs", [])
                            if str(ref).strip()
                        ],
                    }
                ),
        ...
        staged_item["occurrence_count"] = (
            int(staged_item.get("occurrence_count", 1)) + 1
        )
        if _severity_rank(severity) > _severity_rank(
            staged_item.get("severity", "info")
        ):
            staged_item["severity"] = severity
        staged_item["validated"] = bool(staged_item.get("validated")) or bool(
            finding.get("validated")
        )
        ...
        staged_item["service_fingerprint"] = staged_item.get(
            "service_fingerprint"
        ) or finding.get("service_fingerprint")
        staged_item["description"] = _merge_text(
            staged_item.get("description", ""), finding.get("description", "")
        )
        staged_item["remediation"] = _merge_text(
            staged_item.get("remediation", ""), finding.get("remediation", "")
        )
        staged_item["proof"] = _merge_text(
            staged_item.get("proof", ""), finding.get("proof", "")
        )
        staged_item["validation_method"] = _merge_text(
            staged_item.get("validation_method", ""),
            finding.get("validation_method", ""),
        )
        staged_item["confidence_reason"] = _merge_text(
            staged_item.get("confidence_reason", ""),
            finding.get("confidence_reason", ""),
        )
        staged_item["asset_refs"] = sorted(
            {
                *staged_item.get("asset_refs", []),
                *[
                    str(ref)
                    for ref in finding.get("asset_refs", [])
                    if str(ref).strip()
        ...
        staged_item["evidence"] = _dedupe_evidence(
            [*staged_item.get("evidence", []), *normalized_evidence]
        )
        staged_item["corroborating_sources"] = _sort_sources(
            [*staged_item.get("corroborating_sources", []), *sources]
        )
        staged_item["metadata"].update(
            {
                key: value
                for key, value in (finding.get("metadata") or {}).items()
    ...
    for finding_group_id, finding in staged.items():
        ...
        finding["corroborating_sources"] = _sort_sources(
            [*finding.get("corroborating_sources", []), *prior_sources]
        )
        previous_count = (
            int(previous["occurrence_count"])
            if previous and previous.get("occurrence_count")
            else 0
        )
        ...
        finding["first_seen_at"] = (
            ...
            else finding["discovered_at"]
        )
        finding["last_seen_at"] = finding["discovered_at"]
        finding["analyst_status"] = (
            str(previous["analyst_status"])
            if previous and previous.get("analyst_status")
            else "new"
        )
        finding["retest_status"] = (
            str(previous["retest_status"])
            if previous and previous.get("retest_status")
            else "not_requested"
        )
...
def build_finding_groups(findings: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for finding in findings:
        group_id = str(
            ...
            or finding.get("id")
            or _stable_id("group", finding.get("title"), finding.get("target"))
        )
        ...
        if current is None:
            groups[group_id] = {
                ...
                "evidence_count": int(
                    finding.get("evidence_count") or len(finding.get("evidence", []))
                ),
        ...
        current["validated"] = bool(current.get("validated")) or bool(
            finding.get("validated")
        )
        if _severity_rank(finding.get("severity", "info")) > _severity_rank(
            current.get("severity", "info")
        ):
            current["severity"] = _normalize_severity(finding.get("severity"))
        current["last_seen_at"] = max(
            str(current.get("last_seen_at") or ""),
            str(finding.get("last_seen_at") or finding.get("discovered_at") or ""),
        )
        current["first_seen_at"] = min(
            str(current.get("first_seen_at") or ""),
            str(finding.get("first_seen_at") or finding.get("discovered_at") or ""),
        )
        current["occurrence_count"] = max(
            int(current.get("occurrence_count") or 1),
            int(finding.get("occurrence_count") or 1),
        )
        current["evidence_count"] = max(
            int(current.get("evidence_count") or 0),
            int(finding.get("evidence_count") or len(finding.get("evidence", []))),
        )
        current["corroborating_sources"] = _sort_sources(
            [
                *current.get("corroborating_sources", []),
...
def build_asset_summary(
    ...
    for finding in findings:
        asset_id = str(
            ...
            or _stable_id(
                "asset", finding.get("target"), *(finding.get("asset_refs") or [])
            )
        )
        entry = assets.setdefault(
            asset_id,
        ...
        if _severity_rank(finding.get("severity", "info")) > _severity_rank(
            entry.get("highest_severity", "info")
        ):
    ...
    summary.sort(
        key=lambda item: (
            -_severity_rank(item.get("highest_severity", "info")),
            -int(item.get("finding_count", 0)),
...
def build_scan_diff(
    current_findings: List[Dict[str, Any]], previous_findings: List[Dict[str, Any]]
) -> Dict[str, Any]:
    current = {
        str(item.get("finding_group_id") or item.get("id")): item
    ...
    for key in current.keys() & previous.keys():
        ...
        if (
            ...
            or round(float(before.get("confidence") or 0.0), 2)
            != round(float(after.get("confidence") or 0.0), 2)
        ):
