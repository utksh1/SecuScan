...
def parse(output: str) -> Dict[str, Any]:
    ...
    for line in lines:
        if match := port_re.search(line):
            ...
            if state.lower() == "open":
                open_ports.append({"port": port, "protocol": proto, "service": service})
        ...
        findings.append(
            {
                ...
                "description": f"Target has {total_open} active network entry points: "
                + ", ".join(ports_desc),
    ...
    if total_open > 0:
        summary_text.append(
            f"Exposed services: {', '.join([p['service'] for p in open_ports[:5]])}{'...' if total_open > 5 else ''}"
        )
