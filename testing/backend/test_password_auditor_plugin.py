"""
Contract and parser tests for the password_auditor plugin.

These tests load the real plugins/password_auditor/metadata.json, validate it through
...
Related to issue #504: Add parser and contract coverage for plugin `password_auditor`
"""
...
from plugins.password_auditor.parser import parse

PLUGIN_DIR = REPO_ROOT / "plugins" / "password_auditor"
...
def test_password_auditor_metadata_file_exists():
    """metadata.json must exist at the expected plugin path."""
    assert (PLUGIN_DIR / "metadata.json").exists()


def test_password_auditor_metadata_is_valid_json():
...
def test_password_auditor_passes_validator():
    """
...
def test_password_auditor_metadata_id_matches_directory():
    ...
    assert data["id"] == "password_auditor"


def test_password_auditor_engine_is_python3():
...
def test_password_auditor_has_required_target_field():
    """Plugin must declare a required 'target' field for credential auditing."""
    data = json.loads((PLUGIN_DIR / "metadata.json").read_text(encoding="utf-8"))
...
def test_password_auditor_output_parser_is_custom():
    """Parser type must be 'custom', backed by parser.py."""
    data = json.loads((PLUGIN_DIR / "metadata.json").read_text(encoding="utf-8"))
    assert data["output"]["parser"] == "custom"


def test_password_auditor_parser_file_exists():
...
def test_password_auditor_command_renders_with_target(setup_test_environment):
    """
    PluginManager must produce the correct command for password auditing.
    ...
    command = manager.build_command("password_auditor", {"target": "example.com"})
...
def test_password_auditor_loaded_by_plugin_manager(setup_test_environment):
    """PluginManager must successfully load password_auditor from the real plugins directory."""
    manager = PluginManager(str(PLUGINS_DIR))
    asyncio.run(manager.load_plugins())

    plugin = manager.get_plugin("password_auditor")
    assert plugin is not None
    assert plugin.id == "password_auditor"
    assert plugin.name == "Password Auditor"
...
def test_password_auditor_parser_returns_required_keys():
...
def test_password_auditor_parser_count_matches_findings():
    """'count' must equal len(findings)."""
    result = parse(_PASSWORD_AUDITOR_OUTPUT_FIXTURE)
    assert result["count"] == len(result["findings"])


def test_password_auditor_parser_finding_has_required_keys():
...
def test_password_auditor_parser_empty_output():
...
def test_password_auditor_parser_preserves_raw_line_in_metadata():
    """Each finding's metadata.raw must match the original output line."""
    single_line = "admin:password123\n"
    result = parse(single_line)
    assert result["findings"]
    assert result["findings"][0]["metadata"]["raw"] == "admin:password123"
