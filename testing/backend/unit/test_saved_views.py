...
def make_body(name: str, preset: dict = VALID_PRESET, shared: bool = False) -> dict:
    return {"name": name, "filter_json": json.dumps(preset), "shared": shared}
...
@pytest.mark.asyncio
async def test_user_cannot_see_other_users_private_view(app_client: AsyncClient):
    """User B must not see User A's private view."""
    await app_client.post(
        "/api/v1/saved-views",
        json=make_body("User A Private"),
...
@pytest.mark.asyncio
async def test_shared_view_visible_to_other_users(app_client: AsyncClient):
    """A shared view is visible to all users."""
    await app_client.post(
        "/api/v1/saved-views",
    ...
    assert res.json()["views"][0]["name"] == "Shared View"


@pytest.mark.asyncio
async def test_user_cannot_modify_other_users_view(app_client: AsyncClient):
    """User B must get 403 when trying to update User A's view."""
    create_res = await app_client.post(
        "/api/v1/saved-views",
        json=make_body("User A View"),
        headers={"x-user-id": "user-a"},
    )
    view_id = create_res.json()["id"]
...
@pytest.mark.asyncio
async def test_user_cannot_delete_other_users_view(app_client: AsyncClient):
    """User B must get 403 when trying to delete User A's view."""
    create_res = await app_client.post(
        "/api/v1/saved-views",
        json=make_body("User A View"),
        headers={"x-user-id": "user-a"},
...
@pytest.mark.asyncio
async def test_owner_can_modify_own_view(app_client: AsyncClient):
    """Owner can update their own view."""
    create_res = await app_client.post(
        "/api/v1/saved-views",
        json=make_body("My View"),
        headers={"x-user-id": "user-a"},
    )
    view_id = create_res.json()["id"]
...
async def test_owner_can_delete_own_view(app_client: AsyncClient):
    """Owner can delete their own view."""
    create_res = await app_client.post(
        "/api/v1/saved-views",
        json=make_body("My View"),
        headers={"x-user-id": "user-a"},
...
@pytest.mark.asyncio
async def test_shared_view_cannot_be_modified_by_non_owner(app_client: AsyncClient):
    """Even a shared view is read-only to non-owners."""
    create_res = await app_client.post(
        "/api/v1/saved-views",
        json=make_body("Public View", shared=True),
        headers={"x-user-id": "user-a"},
    )
    view_id = create_res.json()["id"]
