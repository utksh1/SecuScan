...
def test_should_run_when_no_last_run(scheduler):
    ...
    assert scheduler._should_run_interval(_now(), None, 3600) is True
...
def test_should_run_when_elapsed_exceeds_schedule(scheduler):
    ...
    assert scheduler._should_run_interval(_now(), last, 3600) is True
...
def test_should_not_run_when_elapsed_below_schedule(scheduler):
    ...
    assert scheduler._should_run_interval(_now(), last, 3600) is False
...
def test_sqlite_naive_datetime_does_not_raise(scheduler):
    ...
    try:
        result = scheduler._should_run_interval(now, sqlite_format, 3600)
...
def test_empty_string_treated_as_no_last_run(scheduler):
    ...
    assert scheduler._should_run_interval(_now(), "", 3600) is True