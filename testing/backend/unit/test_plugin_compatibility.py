...
def test_legacy_plugin_id_aliases_resolve(setup_test_environment):
    ...
    for legacy_id, canonical_id in LEGACY_PLUGIN_ID_ALIASES.items():
        # Only verify if the canonical plugin is actually loaded/available on disk
        plugin_by_canonical = manager.get_plugin(canonical_id)
        if plugin_by_canonical is None:
            continue
        ...
        assert plugin_by_legacy.id in (legacy_id, canonical_id), f"Legacy ID {legacy_id} resolved to {plugin_by_legacy.id} instead of {legacy_id} or {canonical_id}"
...
def test_build_command_with_legacy_id(setup_test_environment):
    ...
    # Test domain-finder / domain_finder (present on disk)
    cmd_legacy = manager.build_command("domain-finder", {"target": "example.com"})
    cmd_canonical = manager.build_command("domain_finder", {"target": "example.com"})
    assert cmd_legacy == cmd_canonical
    assert cmd_legacy is not None
    assert "amass" in cmd_legacy or "python3" in cmd_legacy
    ...
    assert cmd_legacy == cmd_canonical
    assert cmd_legacy is not None
    assert "google" in cmd_legacy or "python" in cmd_legacy or "python3" in cmd_legacy
...
def test_task_start_via_api_resolves_legacy_id(test_client):
    ...
    # Submit task with legacy ID "domain-finder"
    response = test_client.post(
        "/api/v1/task/start",
        json={
            "plugin_id": "domain-finder",
    ...
    assert status_data["plugin_id"] == "domain_finder"
