"""
Contract and parser tests for the dnsx plugin.

These tests load the real plugins/dnsx/metadata.json, validate it
...
Related to issue #495: Add parser and contract coverage for plugin `dnsx`
"""
...
from plugins.dnsx.parser import parse

PLUGIN_DIR = REPO_ROOT / "plugins" / "dnsx"
...
def test_dnsx_passes_validator():
    """
...
def test_dnsx_metadata_id_matches_directory():
...
def test_dnsx_engine_is_dnsx():
    """Engine binary must be 'dnsx' for DNS resolution."""
    data = json.loads((PLUGIN_DIR / "metadata.json").read_text(encoding="utf-8"))
    assert data["engine"]["type"] == "cli"
    assert data["engine"]["binary"] == "dnsx"


def test_dnsx_has_required_target_field():
...
def test_dnsx_command_renders_with_target(setup_test_environment):
    """
    PluginManager must produce the correct dnsx command for a domain.
    """
    manager = PluginManager(str(PLUGINS_DIR))
    asyncio.run(manager.load_plugins())

    command = manager.build_command("dnsx", {"target": "secuscan.in"})
...
def test_dnsx_command_full_token_sequence(setup_test_environment):
...
def test_dnsx_drops_target_token_when_absent(setup_test_environment):
    """
    ...
    rendered = manager.build_command("dnsx", {})

    assert rendered is not None
    assert not any("{" in token for token in rendered), "Unresolved placeholder leaked"
    assert rendered == ["dnsx", "-d", "-silent"]

    populated = manager.build_command("dnsx", {"target": "secuscan.in"})
    assert "secuscan.in" in populated
    assert len(populated) == len(rendered) + 1


def test_dnsx_loaded_by_plugin_manager(setup_test_environment):
    """PluginManager must successfully load dnsx from the real plugins directory."""
    manager = PluginManager(str(PLUGINS_DIR))
    asyncio.run(manager.load_plugins())

    plugin = manager.get_plugin("dnsx")
    assert plugin is not None
    assert plugin.id == "dnsx"
...
def test_dnsx_parser_returns_required_keys():
    """parse() must return a dict with 'findings', 'count', and 'items' keys."""
    result = parse(_DNSX_OUTPUT_FIXTURE)
...
def test_dnsx_parser_count_matches_findings():
    """'count' must equal len(findings)."""
    result = parse(_DNSX_OUTPUT_FIXTURE)
    assert result["count"] == len(result["findings"])


def test_dnsx_parser_finding_has_required_keys():
...
def test_dnsx_parser_severity_classification():
...
def test_dnsx_parser_empty_output():
...
def test_dnsx_parser_preserves_raw_line_in_metadata():
