...
class QualityGate {
  ...
  checkRouteIntegrity() {
    ...
    const routeObjectMatch = routeSource.match(
      /export const routes = \{([\s\S]*?)\} as const/,
    );
    if (!routeObjectMatch) {
      this.fail(
        "Route map exists",
        "Could not parse routes object in src/routes.ts",
      );
      return;
    }
    ...
    for (const match of appSource.matchAll(
      /<Route\s+path=\{routes\.(\w+)\}/g,
    )) {
      const key = match[1];
      if (!routeMap[key]) {
        this.fail(
          "Registered routes resolve",
          `Route key routes.${key} missing in routes.ts`,
        );
        return;
      }
    ...
    for (const file of sourceFiles) {
      ...
      for (const match of content.matchAll(
        ...
        refs.push(match[1]);
      for (const match of content.matchAll(/to=\s*\{\s*`(\/[^`]+)`\s*\}/g))
        refs.push(match[1]);
      ...
      for (const match of content.matchAll(
        /(?:to=\s*\{\s*routePath\.(\w+)\(|navigate\(\s*routePath\.(\w+)\()/g,
      )) {
        const key = match[1] || match[2];
        if (!routeMap[key]) {
          unresolvedRefs.push(
            `${file}: routePath.${key} has no matching routes.${key}`,
          );
          continue;
        }
        refs.push(routeMap[key]);
      ...
      for (const ref of refs) {
        ...
        if (normalized === "/tasks" || normalized.startsWith("/tasks/")) {
          deprecatedRefs.push(
            `${file}: uses deprecated frontend task route "${ref}"`,
          );
          continue;
        }
        const isKnown = routeMatchers.some((matcher) =>
    ...
    if (deprecatedRefs.length > 0) {
      this.fail(
        "No deprecated frontend routes",
        deprecatedRefs.slice(0, 6).join(" | "),
      );
    } else {
  ...
  checkMotionControl() {
    ...
    if (
      css.match(/@keyframes\s+\w*bounce/i) ||
      css.match(/cubic-bezier.*elastic/i)
    ) {
    ...
    const durations =
      css.match(/animation:\s*[^;]*\s+(\d+(?:\.\d+)?)(ms|s)/g) || [];
    let maxInteractiveDuration = 0;

    durations.forEach((d) => {
      // Skip background animations (allowed to be slow)
      if (
        d.includes("gridFloat") ||
        d.includes("scanPulse") ||
        d.includes("lineFloat") ||
        d.includes("scanline")
      ) {
        return;
      }
    ...
    if (maxInteractiveDuration > 300) {
      this.warn(
        "Interactive animations ≤300ms",
        `Found ${maxInteractiveDuration}ms animation`,
      );
    } else {
  ...
  check3DSafety() {
    ...
    if (componentCount > 3) {
      this.fail(
        "Single global 3D layer",
        `Found ${componentCount} background component types`,
      );
    } else {
    ...
    if (css.includes("@media (prefers-reduced-motion")) {
      ...
      this.fail(
        ...
        "Missing @media (prefers-reduced-motion)",
      );
    }

    // Check background opacity
    const bgOpacities =
      css.match(/\.background[^}]*opacity:\s*([0-9.]+)/g) || [];
  ...
  run() {
    ...
    this.log(
      `${COLORS.bold}${COLORS.cyan}║   SecuScan Frontend Quality Gate      ║${COLORS.reset}`,
    );
    this.log(
      `${COLORS.bold}${COLORS.cyan}╚════════════════════════════════════════╝${COLORS.reset}\n`,
    );
