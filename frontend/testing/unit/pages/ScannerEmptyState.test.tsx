...
describe("Scanner empty-state UX", () => {
  ...
  it("shows search-zero and category-zero guidance", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: /Recon Tools/i }),
    );
    await screen.findByText(/WHOIS Lookup/i);
    await user.type(
      screen.getByPlaceholderText("SEARCH_PROTOCOLS..."),
      "nothing-will-match",
    );
    ...
    expect(
      screen.queryByText(/No tools match search/i),
    ).not.toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: /Robots/i }));
    await waitFor(() => {
      expect(
        screen.getByText(/No tools available in this category/i),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /^Quick Start$/i }),
      ).toBeInTheDocument();
    });
  });
});
