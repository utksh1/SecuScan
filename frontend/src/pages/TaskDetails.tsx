...
export default function TaskDetails() {
  ...
  const [activeTab, setActiveTab] = useState<
    "summary" | "results" | "parameters" | "raw"
  >("summary");
  const [expandedFindingRows, setExpandedFindingRows] = useState<
    Record<number, boolean>
  >({});
  const [expandedDiscoveryRows, setExpandedDiscoveryRows] = useState<
    Record<number, boolean>
  >({});
      ...
      >
        <div className="sticky top-0 bg-charcoal-dark/95 backdrop-blur border-b border-white/10 px-8 py-6 flex items-center justify-between z-10">
          <div className="space-y-1">
            <span
              className={`text-[10px] font-black uppercase tracking-[0.2em] px-2 py-0.5 border ${severityTone(finding.severity)}`}
            >
              {finding.severity}
            </span>
            <h2 className="text-xl font-black text-silver-bright italic uppercase tracking-tight">
              {finding.title}
            </h2>
          </div>
          <button
        ...
        <div className="p-8 space-y-10">
          <div className="space-y-4">
            <h3 className="text-[10px] font-black text-silver/30 uppercase tracking-[0.3em] pb-2 border-b border-white/5">
              Description
            </h3>
            <p className="text-sm leading-8 text-silver/85 whitespace-pre-wrap">
              {finding.description}
            </p>
          </div>
          ...
          {finding.remediation && (
            <div className="space-y-4">
              ...
              <div className="bg-rag-green/5 border border-rag-green/20 p-5">
                <p className="text-sm leading-8 text-rag-green/90">
                  {finding.remediation}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-4">
              ...
              <p className="text-sm font-black text-silver-bright uppercase italic">
                {finding.category}
              </p>
            </div>
            {finding.cvss && (
              <div className="space-y-4">
                ...
                <p
                  className={`text-sm font-black italic ${finding.cvss >= 7 ? "text-rag-red" : finding.cvss >= 4 ? "text-rag-amber" : "text-rag-blue"}`}
                >
            ...
            {finding.cve && (
              <div className="space-y-4">
                ...
                <p className="text-sm font-mono text-rag-blue/80 underline cursor-pointer">
                  {finding.cve}
                </p>
              </div>
            )}
            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-silver/30 uppercase tracking-[0.36em] pb-2 border-b border-white/5">
                Detected At
              </h3>
              <p className="text-xs text-silver/60 font-mono italic">
                {finding.discovered_at
                  ? formatDateLong(finding.discovered_at)
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                {Object.entries(finding.metadata || {}).map(([key, val]) => (
                  <div
                    key={key}
                    className="flex justify-between items-start text-[11px] border-b border-white/[0.03] pb-2"
                  >
                    <span className="text-silver/40 uppercase tracking-wider">
                      {formatKeyLabel(key)}
                    </span>
                    <span className="text-silver/70 font-mono break-all text-right max-w-[240px]">
                      {formatValue(val)}
                    </span>
                  </div>
                ))}
      ...
      return (
        <div className="min-h-screen bg-charcoal-dark flex items-center justify-center p-12">
          <div className="max-w-xl w-full bg-charcoal border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] space-y-4 text-center">
            <p className="text-xs font-black text-rag-red uppercase tracking-[0.4em] italic">
              Task_Load_Failed
            </p>
            <p className="text-sm text-silver-bright font-mono break-words">
              {error}
            </p>
            <button
              onClick={() => {
  ...
  const completedTime = task.completed_at
    ? formatLocaleTime(task.completed_at, {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "--:--";
  ...
  const filteredRawLines = rawSearch
    ? rawLines.filter((line: string) =>
        line.toLowerCase().includes(rawSearch.toLowerCase()),
      )
    : rawLines;
  ...
  const severityTone = (severity?: string) => {
    ...
    if (normalized === "critical")
      return "text-rag-red border-rag-red/30 bg-rag-red/10";
    if (normalized === "high")
      return "text-rag-amber border-rag-amber/30 bg-rag-amber/10";
    if (normalized === "medium")
      return "text-rag-blue border-rag-blue/30 bg-rag-blue/10";
    if (normalized === "low")
      return "text-rag-green border-rag-green/30 bg-rag-green/10";
    return "text-silver/65 border-white/10 bg-white/[0.02]";
  };
  const primaryDetail =
    findInputValue("source_ip", "ip", "host", "hostname") || task.target;
  const primaryDetailLabel =
    task.inputs?.source_ip || task.inputs?.ip ? "Source IP" : "Target";
  ...
  const parameterEntries = [
    ...
    [
      "Finish Time",
      task.completed_at ? formatDateLong(task.completed_at) : "ACTIVE",
    ],
    ["Duration", durationLabel],
    [
      "Protocol",
      parsedTarget?.protocol?.replace(":", "").toUpperCase() || "N/A",
    ],
    ...
    [
      "Findings",
      String(result?.structured?.total_count || findings.length).padStart(
        2,
        "0",
      ),
    ],
    ...Object.entries(task.inputs || {}).map(
      ([key, val]) =>
        [formatKeyLabel(key), formatValue(val)] as [string, string],
    ),
  ];
  ...
  const dominantSeverity =
    orderedSeverities.find((level) => (severityCounts[level] || 0) > 0) ||
    "info";
  const providedInputKeys = new Set(Object.keys(task.inputs || {}));
  const presetInputs =
    task.preset && schema?.presets?.[task.preset]
      ? schema.presets[task.preset]
      : {};
  const schemaDefaults = (schema?.fields || []).reduce<Record<string, unknown>>(
    (acc, field) => {
  ...
  const describedParameterEntries = (schema?.fields || [])
    .map((field) => {
      const value = effectiveInputs[field.id];
      const isProvided = providedInputKeys.has(field.id);
      const isPresetValue =
        !isProvided &&
        Object.prototype.hasOwnProperty.call(presetInputs || {}, field.id);
      const source = isProvided
        ? "INPUT"
        : isPresetValue
          ? "PRESET"
          : "DEFAULT";
  ...
  const extraParameterEntries = Object.entries(task.inputs || {})
    .filter(
      ([key]) => !(schema?.fields || []).some((field) => field.id === key),
    )
    .map(([key, value]) => ({
      key,
  ...
  const effectiveParameterEntries = [
    {
      ...
      value: task.target,
      source: "RUNTIME",
      help: "Resolved task target used for the scan.",
    },
    {
      key: "tool",
      label: "Tool",
      ...
      ? [
          {
            ...
            value: task.preset,
            source: "RUNTIME",
            help: "Preset selected when the task was launched.",
          },
        ]
      : []),
  ...
  const DetailCard = ({
    ...
    <div className="bg-charcoal border border-white/5 p-5 shadow-[0_0_0_1px_rgba(255,255,255,0.02)] min-h-[118px] flex flex-col justify-between">
      <div className="space-y-3">
        <span className="text-[10px] font-black text-silver/35 uppercase tracking-[0.28em] italic block">
          {label}
        </span>
        <div className="text-xl md:text-2xl font-black text-silver-bright italic tracking-tight break-words">
          {value}
        </div>
      </div>
      {subValue && (
        <div className="pt-4 text-[9px] font-mono text-rag-blue/90 font-black uppercase tracking-[0.22em]">
          {subValue}
        </div>
      )}
    </div>
    ...
    <div className="min-h-screen bg-charcoal-dark text-silver px-3 py-6 md:px-4 xl:px-5 md:py-8 space-y-8">
            ...
            <div className="space-y-3">
              ...
              <h1 className="text-4xl md:text-6xl text-silver-bright uppercase tracking-tight leading-none italic font-black">
                ...
                <span
                  ...
                  style={{
                    WebkitTextStroke: "1.5px var(--accent-silver-bright)",
                  }}
                >
                  Briefing
                </span>
          ...
          <div className="flex flex-wrap gap-3 xl:justify-end">
            ...
            {task.status === "completed" && (
              <>
                <button
                  onClick={() =>
                    window.open(`${API_BASE}/task/${taskId}/report/html`)
                  }
                ...
                <button
                  onClick={() =>
                    window.open(`${API_BASE}/task/${taskId}/report/csv`)
                  }
                  className="bg-charcoal px-5 py-3 border border-white/10 text-[10px] font-black uppercase tracking-[0.26em] italic transition-colors hover:bg-white/[0.04] flex items-center gap-2"
                >
                ...
                <button
                  onClick={() =>
                    window.open(`${API_BASE}/task/${taskId}/report/pdf`)
                  }
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <DetailCard
          ...
          value={String(
            result?.structured?.total_count || findings.length,
          ).padStart(2, "0")}
            ...
            {activeTab === "summary" && (
              ...
              >
                <motion.div
                  ...
                  className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.5fr)_420px] gap-6"
                >
                ...
                <motion.div
                  variants={itemVariants}
                  className="border border-white/8 bg-charcoal p-6"
                >
                  <div className="flex items-center gap-4 mb-5">
                    <h3 className="text-xs font-black text-silver-bright uppercase tracking-[0.36em] italic">
                      Priority Findings
                    ...
                    <span className="text-[10px] uppercase tracking-[0.24em] text-silver/40">
                      {previewFindings.length} Top Hits
                    </span>
                  </div>
                  {previewFindings.length > 0 ? (
                      ...
                      {previewFindings.map((f: any, idx: number) => (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start mb-3">
                            <span
                              className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 border ${severityTone(f.severity)}`}
                            >
                              {f.severity}
                            </span>
                            <span className="text-[10px] font-mono text-silver/20 group-hover:text-rag-blue transition-colors">
                              #{idx.toString().padStart(3, "0")}
                            </span>
                          </div>
                          <h4 className="text-sm font-black text-silver-bright uppercase italic mb-2 line-clamp-1">
                            {stripAnsi(f.title)}
                          </h4>
                          <p className="text-xs text-silver/50 line-clamp-2 leading-relaxed">
                    ...
                    <p className="text-sm text-silver/55 italic">
                      No findings identified for this target profile.
                    </p>
                  )}
                </motion.div>
              </motion.section>
              ...
              >
                ...
                >
                  <div className="flex items-center gap-4 mb-5">
                    ...
                    <span className="text-[10px] uppercase tracking-[0.24em] text-silver/40">
                      {resultEntryCount}{" "}
                      {resultEntryCount === 1 ? "Entry" : "Entries"}
                    </span>
                  </div>
                  {tableRows.length > 0 ? (
                        ...
                        <thead>
                          <tr className="sticky top-0 z-20 border-b border-white/10 text-silver/40 uppercase tracking-[0.22em] bg-[#0c0c0f] shadow-[0_1px_0_0_rgba(255,255,255,0.05)]">
                            {Object.keys(tableRows[0]).map((key, kIdx) => (
                              <th
                                key={key}
                                className={`px-4 py-4 font-black ${kIdx === 0 ? "w-[120px]" : ""}`}
                              >
                                {formatKeyLabel(key)}
                              </th>
                            ))}
                              ...
                              >
                                {Object.entries(row).map(([key, val], vIdx) => {
                                  ...
                                  return (
                                    <td
                                      key={vIdx}
                                      className={`px-4 py-4 align-top ${vIdx === 0 ? "text-rag-blue font-bold" : "text-silver/75"}`}
                                    >
                                      <div className="space-y-2">
                                        <div
                                          className={`${!isExpanded && isLong ? "line-clamp-2" : ""} break-words whitespace-pre-wrap`}
                                        >
                                          {strVal}
                                        </div>
                                        {isLong && vIdx > 0 && (
                          ...
                          {findings.map((f: Finding, idx: number) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-4 py-6 align-top">
                                  <span
                                    className={`inline-flex px-3 py-1 text-[10px] font-black uppercase italic border shadow-sm ${severityTone(f.severity)}`}
                                  >
                                    {f.severity || "info"}
                                  </span>
                                </td>
            ...
            {activeTab === "parameters" && (
              ...
              >
                {result?.command_used && (
                  <motion.div
                    variants={itemVariants}
                    className="border border-rag-blue/20 bg-charcoal p-6"
                  >
                    <div className="flex items-center gap-4 mb-5">
                      <h3 className="text-xs font-black text-silver-bright uppercase tracking-[0.36em] italic">
                        Final Command
                ...
                >
                  ...
                  <p className="text-[10px] text-silver/40 uppercase tracking-[0.2em] mb-5">
                    Shows the final scan configuration, including defaults and
                    preset values applied at runtime.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    {effectiveParameterEntries.map((entry) => (
                      <div
                        key={entry.key}
                        className="border border-white/6 bg-black/20 px-4 py-4 min-h-[130px]"
                      >
                        ...
                        <p
                          className={`text-sm font-black uppercase break-words leading-6 ${
              ...
              >
                ...
                >
                  ...
                  <div className="border border-white/6 bg-black/30 p-4 max-h-[720px] overflow-auto">
                    <pre
                      className={`${wrapRawOutput ? "whitespace-pre-wrap break-words" : "whitespace-pre"} text-[11px] leading-6 font-mono text-silver/75`}
                    >
          ...
          <section className="border border-white/8 bg-charcoal p-5 space-y-5">
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={label}
                  className="border-b border-white/6 pb-3 last:border-0 last:pb-0"
                >
                  <p className="text-[10px] uppercase tracking-[0.24em] text-silver/30 font-black mb-1">
                    {label}
                  </p>
                  <p className="text-sm text-silver-bright font-mono break-words">
                    {value}
          ...
          <section className="border border-white/8 bg-charcoal p-5 space-y-5">
            ...
            <div className="space-y-3">
              {effectiveParameterEntries.slice(0, 6).map((entry) => (
                <div
                  key={entry.key}
                  className="flex items-start justify-between gap-4 border-b border-white/6 pb-3 last:border-0 last:pb-0"
                >
      ...
      <footer className="pt-12 border-t border-white/6 flex flex-col md:flex-row justify-between items-center gap-6 text-[9px] font-black uppercase tracking-[0.4em] italic opacity-25">
        <div className="flex items-center gap-6">
          ...
          CLASSIFIED_EXECUTIVE_SUMMARY // CORE_DAEMON_LOG_ID::
          {taskId?.split("-")[0].toUpperCase()}
        </div>
        <div className="flex gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="w-20 h-1 bg-silver/20"></div>
          ))}
        </div>
      </footer>
      ...
      <AnimatePresence>
        {selectedFinding && (
          <>
            ...
            />
            <FindingDrawer
              finding={selectedFinding as Finding}
