...
export default function Reports() {
  ...
  const [selectedType, setSelectedType] = useState(() =>
    getPreference("reports-type-filter", "all"),
  );
  const [selectedStatus, setSelectedStatus] = useState(() =>
    getPreference("reports-status-filter", "all"),
  );
  const [selectedDateRange, setSelectedDateRange] = useState(() =>
    getPreference("reports-date-filter", "all"),
  );
    ...
    <div className="min-h-screen bg-charcoal-dark text-silver p-6 md:p-12 space-y-12">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          ...
          {
            label: "Surface_Nodes",
            val: summary.total_assets || 0,
            color: "bg-rag-green",
            unit: "NODES",
          },
          {
            label: "Aggregate_Anomalies",
            val: summary.total_findings || 0,
            color: "bg-rag-red",
            unit: "TRIGGERS",
          },
          {
            label: "Archive_Volume",
            val: "12.4",
            color: "bg-rag-amber",
            unit: "GB",
          },
          ...
          <div
            key={i}
            className={`${m.color} border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between h-40 group hover:-translate-y-1 transition-transform`}
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-12">
        ...
        <aside className="xl:col-span-1 space-y-12">
          <section className="bg-charcoal border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] space-y-8">
            <div className="space-y-4">
              <label className="text-[10px] font-black text-silver-bright uppercase tracking-[0.2em] italic">
                Classification_Isolation
              </label>
            ...
            <div className="p-8 border-4 border-black border-dashed space-y-4 bg-charcoal-dark/50">
              <div className="flex items-center gap-3">
                ...
                />
                <h4 className="text-[10px] font-black text-silver-bright uppercase tracking-[0.2em] italic leading-none">
                  Integrity_Secure
                </h4>
              </div>
              <p className="text-[10px] text-silver/40 font-black uppercase tracking-widest leading-loose italic">
                Dossiers are cryptographically hashed and recorded.
                Modifications are strictly detectable by the Enclave audit
                daemon.
              </p>
            </div>
          </section>
        ...
        <main className="xl:col-span-3 space-y-8">
          {error && (
            <div className="p-8 border-4 border-rag-red bg-charcoal-dark space-y-4 shadow-[8px_8px_0px_0px_rgba(255,0,0,0.1)]">
              <p className="text-sm font-black text-rag-red uppercase tracking-widest">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b-4 border-black pb-8">
            <h2 className="text-5xl font-black text-silver-bright italic uppercase tracking-tighter shrink-0">
              Historical_Ledger
            </h2>
            <div className="h-0.5 bg-black/10 flex-1 mb-2 hidden md:block"></div>
            <span className="text-[10px] font-mono text-silver/20 uppercase font-black mb-2 animate-pulse">
                ...
                <div className="col-span-2 py-40 border-4 border-dashed border-black/5 text-center flex flex-col items-center gap-8 bg-charcoal/30">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-mono text-silver/10 uppercase tracking-widest leading-relaxed">
                      No reports match the current filter. Adjust classification
                      to view additional dossiers.
                    </p>
                  </div>
                </div>
              ) : (
                      ...
                      className={`absolute top-0 left-0 h-2 transition-all duration-500 ${
                        report.status === "ready"
                          ...
                          : report.status === "failed"
                            ? "bg-rag-red w-full"
                            : "bg-rag-amber w-1/2 animate-pulse"
                      }`}
                    ></div>
                    ...
                    <div className="space-y-8 relative z-10">
                      <div className="flex justify-between items-start">
                        ...
                        <ReportIcon
                          icon={File01Icon}
                          size={24}
                          className="text-silver/10 group-hover:text-silver-bright transition-colors"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-6 py-6 border-y-2 border-black border-dashed">
                        <div className="space-y-1">
                          <span className="text-[8px] font-black text-silver/20 uppercase tracking-widest italic block">
                            Findings
                          </span>
                          <span className="text-xs font-black font-mono text-silver-bright">
                            {report.findings.toString().padStart(3, "0")}
                          </span>
                        </div>
                        <div className="space-y-1 text-center">
                          ...
                          <span className="text-xs font-black font-mono text-silver-bright">
                            {report.assets.toString().padStart(3, "0")}
                          </span>
                        </div>
                        <div className="space-y-1 text-right">
                          ...
                          <span className="text-xs font-black font-mono text-silver-bright">
                            {report.pages.toString().padStart(3, "0")}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-end pt-2">
                        <div className="space-y-1">
                          <p className="text-[8px] font-black uppercase text-silver/20 tracking-[0.3em] italic leading-none">
                            TIMESTAMP
                          </p>
                        ...
                        <div className="flex gap-4">
                          ...
                          {(() => {
                            const formats = ["pdf", "html", "csv"] as const;
                            const orderedFormats = preferred && formats.includes(preferred as any)
                            ...
                            return orderedFormats.map((format) => (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  window.open(
                                    `${API_BASE}/task/${report.task_id}/report/${format}`,
                                    "_blank",
                                  );
                                }}
                                disabled={report.status === "generating"}
                                className={`border-4 border-black p-3 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                                  preferred === format
                                    ? "bg-rag-amber text-black font-black"
      ...
      <footer className="pt-24 border-t-4 border-black/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[9px] font-black uppercase tracking-[0.5em] italic opacity-20">
        <div className="flex items-center gap-6">
          ...
          RESTRICTED_ACCESS_ENCLAVE // SYSTEM_ARCHIVE_DAEMON //{" "}
