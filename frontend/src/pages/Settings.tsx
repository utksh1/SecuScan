...
export default function Settings() {
  ...
  const handleExport = () => {
    ...
    downloadAnchorNode.setAttribute(
      "download",
      `secuscan_config_${new Date().toISOString().split("T")[0]}.json`,
    );
  ...
  const InputField = ({
    ...
    <div className="bg-charcoal border-4 border-black p-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] transition-all group">
      <div className="space-y-2 mb-6">
        <label className="text-[10px] font-black text-silver-bright uppercase tracking-[0.2em] block italic group-hover:text-rag-blue transition-colors">
          {label}
        </label>
        <p className="text-[9px] text-silver/40 uppercase font-mono font-bold tracking-widest leading-relaxed">
          {description}
        </p>
      </div>
      <input
      ...
      >
        {options.map((opt: any) => (
          <option
            key={opt.value}
            value={opt.value}
            className="bg-charcoal text-silver-bright"
          >
            {opt.label}
          </option>
        ))}
  ...
  const Toggle = ({ checked, onChange, label, description }: any) => (
    ...
    >
      <div className="space-y-2 text-left mr-8">
        <label className="text-[10px] font-black text-silver-bright uppercase tracking-widest block group-hover:text-rag-green transition-colors">
          {label}
        </label>
        <span className="text-[9px] text-silver/30 uppercase tracking-tighter italic font-mono font-bold leading-relaxed">
          {description}
        </span>
      </div>
      <div
        className={`w-14 h-7 border-4 border-black relative shrink-0 transition-all ${checked ? "bg-rag-green" : "bg-charcoal-dark"}`}
      >
        <div
          className={`absolute top-0 w-5 h-full bg-black transition-all ${checked ? "left-7" : "left-0"}`}
        ></div>
      </div>
    </button>
  );
      ...
      <header className="relative flex flex-col md:flex-row justify-between items-start md:items-end gap-8 pb-12 border-b-4 border-silver-bright/10 font-black">
        ...
        <div className="flex flex-col items-end gap-4">
          <div className="bg-charcoal border-4 border-black px-8 py-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <span className="text-[10px] font-black text-silver/20 uppercase tracking-[0.4em] block mb-1 italic">
              SYSTEM_TIMEZONE_SYNC
            </span>
            <span className="text-xs font-black font-mono text-rag-blue tracking-widest italic">
              {systemTimezone.toUpperCase()}
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-12 pt-4">
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <SelectField
                ...
                onChange={(val: string) =>
                  setConfig({ ...config, scanIntensity: val })
                }
                options={[
              ...
              <SelectField
                ...
                onChange={(val: number) =>
                  setConfig({ ...config, dataRetention: val })
                }
                options={[
              ...
              <InputField
                ...
                onChange={(val: number) =>
                  setConfig({ ...config, concurrentScans: val })
                }
              />
              <InputField
                ...
                onChange={(val: number) =>
                  setConfig({ ...config, scanTimeout: val })
                }
              />
            </div>
          </section>
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <InputField
                ...
                onChange={(val: string) =>
                  setConfig({ ...config, shodanKey: val })
                }
              />
              <InputField
                ...
                onChange={(val: string) =>
                  setConfig({ ...config, virustotalKey: val })
                }
              />
            </div>
          </section>
          ...
          <section className="space-y-8">
            ...
            <div className="bg-charcoal border-4 border-black p-10 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-silver-bright uppercase tracking-widest block italic">
                  Authorized_Ingress_Vectors
                </label>
                <p className="text-[10px] text-silver/40 uppercase font-bold italic mb-6 leading-relaxed">
                  Line-delimited IP/CIDR whitelist for high-privilege access
                </p>
              </div>
              <textarea
                value={config.ipWhitelist}
                onChange={(e) =>
                  setConfig({ ...config, ipWhitelist: e.target.value })
                }
                rows={4}
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Toggle
                ...
                onChange={(val: boolean) =>
                  setConfig({ ...config, autoRescanCritical: val })
                }
              />
              <Toggle
                ...
                onChange={(val: boolean) =>
                  setConfig({ ...config, autoPurgeFailed: val })
                }
              />
            </div>
          </section>
        ...
        <aside className="xl:col-span-1 space-y-12">
          <section className="bg-charcoal border-4 border-black p-10 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] space-y-6">
            <h3 className="text-[11px] font-black text-silver-bright uppercase tracking-[0.5em] italic mb-8">
              Management_Tools
            </h3>
            <div className="space-y-4">
              <button
          ...
          <section className="bg-charcoal-dark border-4 border-black p-10 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] space-y-6">
            <div className="space-y-4">
              <h3 className="text-[11px] font-black text-silver-bright uppercase tracking-[0.5em] italic border-b-4 border-black pb-4">
                Engine_Status
              </h3>
              <div className="space-y-4 font-mono">
                <div className="flex justify-between text-[10px]">
                  <span className="text-silver/30 uppercase tracking-tighter">
                    Engine Version
                  </span>
      ...
      <footer className="pt-24 border-t-4 border-black/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[9px] font-black uppercase tracking-[0.5em] italic opacity-20">
        ...
        <div className="flex gap-4">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div key={i} className="w-2 h-2 bg-silver/20 rounded-full"></div>
          ))}
        </div>
      </footer>
      {showResetModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-charcoal border-4 border-rag-amber p-8 max-w-md w-full space-y-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <p className="text-silver/70 text-sm">
              This will restore factory settings and remove all custom
              configurations.
            </p>
      ...
      {showPurgeModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-charcoal border-4 border-rag-red p-8 max-w-md w-full space-y-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <p className="text-silver/70 text-sm">
              This will permanently delete ALL history and assets. This action
              cannot be undone.
            </p>
