...
const severityChip: Record<string, string> = {
  ...
  low: 'bg-[var(--bg-primary)] text-[var(--text-secondary)]-bright border border-silver-bright/15',
  info: 'bg-[var(--bg-primary)] text-[var(--text-secondary)] border border-silver/15',
}
...
function FindingRow({ item, showBaseline, showComparison }: {
  ...
  return (
    <div className="border-2 border-black bg-[var(--bg-primary)]/50 p-4 space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        <span className={`px-2 py-0.5 text-[9px] font-black uppercase border-2 border-black ${chip}`}>
          {severity}
        </span>
        <span className="text-[9px] font-mono text-[var(--text-secondary)]/40 uppercase">{finding.category}</span>
      </div>
      <p className="text-sm font-black text-[var(--text-secondary)]-bright uppercase tracking-tight">{finding.title}</p>
      <p className="text-[10px] font-mono text-[var(--text-secondary)]/50">{finding.target}</p>
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between border-b-2 border-black pb-2">
        ...
        <span className="text-[10px] font-mono text-[var(--text-secondary)]/40">{items.length}</span>
      </div>
      {items.length === 0 ? (
        <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]/30 italic">None</p>
      ) : (
...
export default function ReportCompare() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-secondary)] p-6 md:p-12 space-y-10">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pb-10 border-b-4 border-silver-bright/10">
        <div className="space-y-4">
          <Link
            to={routes.reports}
            className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]/50 hover:text-[var(--text-secondary)]-bright"
          >
      ...
      {loading && (
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[var(--text-secondary)]/30 animate-pulse">
        ...
        <>
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <label className="space-y-3 block">
              <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]-bright">
                Baseline report (older)
              </span>
              <select
                value={baselineReportId}
                onChange={(e) => setBaselineReportId(e.target.value)}
                className="w-full bg-charcoal border-4 border-black p-4 text-sm font-mono text-[var(--text-secondary)]-bright"
              >
          ...
          {reports.length < 2 && (
            <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]/40">
          ...
          {comparison && (
            ...
            >
              <div className="flex items-center gap-3 text-[var(--text-secondary)]-bright">
                <HugeiconsIcon icon={Analytics02Icon} size={28} />
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]/40">Diff ready</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <CompareSection
                  ...
                  tone="text-[var(--text-secondary)]-bright"
                  showComparison
                />
                <CompareSection
