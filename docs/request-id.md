...
The request ID lifecycle is managed by [request_context.py](../backend/secuscan/request_context.py) using Python's standard `contextvars.ContextVar`.
...
For HTTP requests, [request_middleware.py](../backend/secuscan/request_middleware.py) defines the `RequestIDMiddleware` registered in FastAPI.
...
- **Filter**: `RequestIDFilter` (in [logging_utils.py](../backend/secuscan/logging_utils.py)) dynamically fetches the current ID using `get_request_id()` and attaches it to the log record as `request_id`.
- **Formatter**: `JSONFormatter` (in [logging_utils.py](../backend/secuscan/logging_utils.py)) serializes the log output as a JSON string, emitting `request_id` as a top-level field.
...
Critical events are permanently recorded in the SQLite database by calling `db.log_audit()` in [database.py](../backend/secuscan/database.py).
...
Example from [workflows.py](../backend/secuscan/workflows.py):
